...
final class EccubeMakeTrait extends AbstractMaker implements InputAwareMakerInterface
{
    ...
    public function generate(InputInterface $input, ConsoleStyle $io, Generator $generator)
    {
        ...
        $classExists = in_array($entityClassDetails->getFullName(), get_declared_traits());
        if (!$classExists) {
            $entityTraitGenerator = new EntityTraitGenerator($generator);
            $entityPath = $entityTraitGenerator->generateEntityTrait(
                $entityClassDetails,
        ...
        if ($classExists) {
            ...
            $io->text([
                'Your trait already exists! So let\'s add some new fields!',
            ]);
        } else {
            $io->text([
                '',
                'Trait generated! Now let\'s add some fields!',
                ...
                switch ($newField->getType()) {
                    case EntityRelation::MANY_TO_ONE:
                        if ($newField->getOwningClass() === $entityClassDetails->getFullName()) {
                            ...
                            var_dump($newField->getOwningClass());
    ...
    private function createClassManipulator(string $path, ConsoleStyle $io, bool $overwrite): TraitSourceManipulator
    {
        $manipulator = new TraitSourceManipulator($this->fileManager->getFileContents($path), $overwrite);
